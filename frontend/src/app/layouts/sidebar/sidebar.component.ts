...
export class SidebarComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  ngOnInit() {
    ...
    this.roles = this.apiservice.getRole();
    this.initialize();
  ...
  _scrollElement() {
    setTimeout(() => {
      if (document.getElementsByClassName("mm-active").length > 0) {
        const currentPosi : any = document.getElementsByClassName("mm-active");
        const currentPosition = currentPosi[0]['offsetTop']
  ...
  initialize(): void {
    ...
    for(let i of MENU){
      if(i.role){
        for(let j of i.role){
          if(this.roles.includes(j)){
            this.menuItems.push(MENU[count])
            break;
          }
        }
      }
